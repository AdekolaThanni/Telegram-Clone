...
function ChatItem({ chatData, activeChat, onClick }) {
  const formattedDate = useTime(chatData.latestMessage.time);
  const chatItemRef = useRef();

  const [chatOptionsVisibility, setChatOptionsVisibility] = useState(false);

  const showOptions = (event) => {
    event.preventDefault();
    setChatOptionsVisibility([
      true,
      {
        event,
      },
    ...
    <>
        ...
        <div className="flex-grow overflow-hidden">
          ...
          <div className="flex justify-between">
            <span className="flex-grow truncate">
              {!chatData.privateChat && chatData.latestMessage.sender && (
                <span
                  className={`text-primary-text ${activeChat && "!text-white"}`}
                >
                  {chatData.latestMessage.sender}:{" "}
                </span>
              )}
      ...
      <AnimatePresence>
        {chatOptionsVisibility && (
          <ChatOptionsModal
            privateChat={chatData.privateChat}
            eventData={chatOptionsVisibility[1]}
